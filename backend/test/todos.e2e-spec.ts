...
describe('TodosController (e2e)', () => {
  ...
  it('/todos/:id (GET) - should return a single todo', async () => {
    const todo = await repository.save({ title: 'test-get-one', description: 'desc-get-one' });
  ...
  it('/todos/:id (PATCH) - should update a todo', async () => {
    const todo = await repository.save({ title: 'test-patch', description: 'desc-patch' });
  ...
  it('/todos/:id (DELETE) - should delete a todo', async () => {
    const todo = await repository.save({ title: 'test-delete', description: 'desc-delete' });
